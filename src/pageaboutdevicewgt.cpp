	...
	m_ui(new Ui::PageAboutDeviceWgtClass)
{
	...
	connection_type = this->devinterface->getProtocolType();
	...
	connect(m_ui->updateFromFileBtn,   SIGNAL(clicked()),                            this,              SLOT(OnUpdateFromFileBtnClicked()));
	connect(m_ui->autoUpdateBtn,       SIGNAL(clicked()),                            this,              SLOT(OnAutoUpdateBtnClicked()));
	connect(m_ui->eepromPrecedenceBox, SIGNAL(toggled(bool)),                        this,              SIGNAL(precedenceCheckedSgn(bool)));
	connect(m_ui->chooseFirmwareBtn,   SIGNAL(clicked()),                            this,              SLOT(CallChooseFirmDlg()));
...
void PageAboutDeviceWgt::OnUpdateFromFileBtnClicked()
{
	if (this->connection_type != dtSerial && this->connection_type != dtNet)
	{
		QMessageBox::warning(this, "Error", "Firmware can only be updated via xi-com (serial port) and xi-net.", QMessageBox::Ok);
		return;
	}
...
void PageAboutDeviceWgt::OnAutoUpdateBtnClicked()
{
	if (this->connection_type != dtSerial && this->connection_type != dtNet)
		{
			QMessageBox::warning(this, "Error", "Firmware can only be updated via xi-com (serial port) and xi-net.", QMessageBox::Ok);
