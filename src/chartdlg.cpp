    ...
    : QDialog(parent)
{
	...
	plots[PWRRATIONUM]->setVisible(ui.averPwrRatioChk->isChecked());
	...
	connect(ui.averPwrRatioChk,         SIGNAL(stateChanged(int)),            this,              SLOT(ChangePlotCnt()));
...
void ChartDlg::plotsReplot()
{
	...
	if(ui.averPwrRatioChk->isChecked())			plots[PWRRATIONUM]->replot();
...
void ChartDlg::CommonPlotSetup()
{
	...
	plots[PWRRATIONUM]->Init(plots, gStgs->averPwrRatioStgs, "%", "Averaged Power Ratio");
...
void ChartDlg::ReinitPlots()
{
	...
	plots[PWRRATIONUM]->ReInit(gStgs->averPwrRatioStgs);
...
void ChartDlg::update(int int_offset)
{
	...
	for(int i = 0; i<datapoints; i++){
		if(curr != NULL){
			...
			if(ui.averPwrRatioChk->isChecked())	data[PWRRATIONUM].push_back(curr->averPowerRatio);
...
void ChartDlg::updatePlotsVisible()
{
	...
	plots_visible[PWRRATIONUM] = ui.averPwrRatioChk->isChecked();
...
void ChartDlg::LoadFile_v10(QString filename)
{
	...
	while(!file.atEnd())
	{
		...
		file.read((char *)&curr->averPowerRatio,	sizeof(double));
...
void ChartDlg::OnExportBtnClicked()
{
	...
	while(curr != NULL)
	{
		...
		write_dbl(file, curr->averPowerRatio,	3);
...
void ChartDlg::ClearPlots(bool first)
{
	if(first){
		...
		if (log->size > 0){
			...
			data[PWRRATIONUM].push_back(log->list->averPowerRatio);
...
//	settings->setValue("SupVoltage_graph_enable",	ui.pwrVoltageChk->isChecked());
	settings->setValue("AverPowerRatio_graph_enable", ui.averPwrRatioChk->isChecked());
...
void ChartDlg::FromSettingsToUi(QSettings *my_settings, QSettings *default_stgs)
{
	...
	if(settings->contains("AverPowerRatio_graph_enable"))
		ui.averPwrRatioChk->setChecked(settings->value("AverPowerRatio_graph_enable", false).toBool());
...
void ChartDlg::setHardware(unsigned int entype)
{
	local_entype = entype;
	{
		...
		if (entype == ENGINE_TYPE_STEP) {
			...
			ui.averPwrRatioChk->setChecked(false);
			ui.averPwrRatioChk->setEnabled(false);
			...
			ui.averPwrRatioChk->setEnabled(true);
