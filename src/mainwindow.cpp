...
void MainWindow::InitUI()
{
	...
	wrmLbl.setFont(font);
	wrmLbl.setPalette(palette_yellow);
	wrmLbl.setText("WRM");
...
void MainWindow::CommandErrorLog(const char* message, result_t result)
{
	...
	{
		case -1:
			result_string = QString("result_error");
			break;
		case -2:
			result_string = QString("result_not_implemented");
			break;
		case -3:
			result_string = QString("result_value_error");
			break;
		case -4:
			result_string = QString("result_nodevice");
			break;
		default:
			result_string = QString("Unexpected value! Must be between -1, -2, -3 or -4");
			break;
	}
	if (inited)
		Log(QString("Function ") + message + QString("() failed, result = ") + result_string 
		  + QString(" (") + toStr((int)result) + QString(")"), SOURCE_XILAB, LOGLEVEL_ERROR);
}
...
void MainWindow::CommandWarningLog(const char* message, result_t result)
{
	...
	if (inited)
		Log(QString("Function ") + message + QString("() failed, result = ") + result_string
		  + QString(" (") + toStr((int)result) + QString(")"), SOURCE_XILAB, LOGLEVEL_WARNING);
}
