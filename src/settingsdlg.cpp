...
const char* PageGraphPwmStr = "Graph_averPowerRatio_setup";
...
void SettingsDlg::AllPagesFromUiToClassToDevice()
{
	...
	((PageQuickStart*)pageWgtsLst[PageQuickStartNum])->updateShowOnStartupSetting();
...
void SettingsDlg::InitControls()
{
	...
	pageWgtsLst.push_back((QWidget*)new PageGraphCommonWgt(NULL, graphStgs->commonStgs));
		...
		PageGraphPwmNum=i++;
		childItem = new QTreeWidgetItem(parentItem2);
		childItem->setText(0, tr("Averaged Power Ratio"));
		treeWgtsLst.push_back(childItem);
		pageWgtsLst.push_back((QWidget*)new PageGraphWgt(NULL, graphStgs->averPwrRatioStgs, PageGraphPwmStr));
		((PageGraphWgt*)pageWgtsLst.back())->SetCaption("Averaged power ratio curve setup");
	...
	//Program configuration->Quick start
	PageQuickStartNum=i++;
	childItem = new QTreeWidgetItem(parentItem);
	childItem->setText(0, tr("Quick start"));
	treeWgtsLst.push_back(childItem);
	pageWgtsLst.push_back((QWidget*)new PageQuickStart(NULL));
...
void SettingsDlg::CheckProfileFwHwVersions()
{
	...
	XSettings settings(temp_config_file, QSettings::IniFormat, QIODevice::ReadOnly);
