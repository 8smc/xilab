...
StartWindow::StartWindow(QWidget *parent): QWidget(parent), m_ui(new Ui::StartWindowClass)
{
	...
	*/
	...
	connect(m_ui->quickStartBtn, SIGNAL(clicked()), this, SLOT(quickStartBtnClicked()));
...
bool StartWindow::return_all_com()
{
	return dss->Enumerate_all_com;
}
...
void StartWindow::quickStartBtnClicked()
{
	emit showQuickStartWindow();
}
