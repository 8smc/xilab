...
@if not exist .\ximc-0.0.tar.gz (
    ...
    powershell "Invoke-WebRequest -Uri http://files.xisupport.com/libximc_alpha/libximc-%XIMC_VER%-all.tar.gz -OutFile ximc-0.0.tar.gz"
    @if not %errorlevel% == 0 goto FAIL
)
7z x -y ximc-*.tar.gz
@if not %errorlevel% == 0 goto FAIL
...
@if not %errorlevel% == 0 goto FAIL
powershell "Invoke-WebRequest -Uri http://files.xisupport.com/xiresource/xiresource-%XIRESOURCE_VERSION%.zip -OutFile xiresource.zip"
@if not %errorlevel% == 0 goto FAIL
7z x -y xiresource.zip
@if not %errorlevel% == 0 goto FAIL
ren xiresource-v%XIRESOURCE_VERSION% xiresource
@if not %errorlevel% == 0 goto FAIL
